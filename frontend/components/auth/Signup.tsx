...
export default function Signup() {
  const { registerUser, registrationLoading, registrationError, signInWithGoogle, googleLoading } = useAuth();
  ...
  const googleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      try {
        ...
        setLoadingMessageIndex(3); // "Creating your account..."
        router.replace("/home");
        showToast("Account created successfully!", "success", "bottom-right");
      } catch (error) {
        showToast("Google Sign Up Failed", "error", "bottom-right");
      } finally {
